...
Searcher {
    ...
    // Fenrir: each app gets its own unit, named after its entry, so running out of memory closes
    // that app rather than the shell and compositor it would otherwise share a cgroup with.
    function launch(entry: DesktopEntry): void {
        appDb.incrementFrequency(entry.id);

        const unit = ["app2unit", "-a", entry.id, "--"];
        if (entry.runInTerminal)
            Quickshell.execDetached({
                command: [...unit, ...GlobalConfig.general.apps.terminal, `${Quickshell.shellDir}/assets/wrap_term_launch.sh`, ...entry.command],
                workingDirectory: entry.workingDirectory
            });
        else
            Quickshell.execDetached({
                command: [...unit, ...entry.command],
                workingDirectory: entry.workingDirectory
            });
    }
